...
`cut -f 3,4,6-8 -d, $joinfile|grep ADNZ | sort |uniq -c |sort -rn >>$statsfile`;

`echo "# pairs output by 5, what is the range of Count_MM and P10/10 values">>$statsfile`;
`cut -f 3-8,19-23 -d, $joinfile|grep ACDNZ | sort |uniq -c |sort -rn >>$statsfile`;

`echo "# pairs output by 5 with 0 Count_MM what is the range of  P10/10 values">>$statsfile`;
`cut -f 3-8,19-23 -d, $joinfile|grep ACDNZ,0,0,0,0,0 | sort |uniq -c |sort -rn >>$statsfile`;

`echo "# pairs output by 5 with 1 Count_MM what is the range of  P10/10 values">>$statsfile`;
`cut -f 3-8,19-23 -d, $joinfile|grep ACDNZ,1,1,1,1,1 | sort |uniq -c |sort -rn >>$statsfile`;

`echo "# pairs output by 5 with 2 Count_MM what is the range of  P10/10 values">>$statsfile`;
`cut -f 3-8,19-23 -d, $joinfile|grep ACDNZ,2,2,2,2,2 | sort |uniq -c |sort -rn >>$statsfile`;
